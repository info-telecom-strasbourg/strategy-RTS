...
void BottomLidar::detection(Vector<Pos> & obstacles, Vector<Pos> & detectables)
{
...
{
    Pos oppon_pos_arr[2];
    Vector<Pos> oppon_pos(oppon_pos_arr);
    for(unsigned int i = 0; i < rob_opponents.size(); i++)
        oppon_pos.push_back(rob_opponents[i].position);

    Pos obstacles_arr[2];
    Vector<Pos> obstacles(obstacles_arr);
    detection(obstacles, oppon_pos);

    return (obstacles.empty()) ? FAST : SLOW;
}
